...
if (!process.env.OPENAI_API_KEY && !process.env.AI_KEY) {
  console.warn('⚠️  ADVERTENCIA: No se detectó API key de OpenAI. Establece OPENAI_API_KEY o AI_KEY en .env')
}
...
aiRouter.get('/summary/:id', async (req, res) => {
  const apiKey = process.env.OPENAI_API_KEY || process.env.AI_KEY
  
  if (!apiKey) {
    return res.status(503).json({ error: 'Servicio no configurado. Falta API key.' })
  }
